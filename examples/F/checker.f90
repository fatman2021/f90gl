...
public :: display, myReshape, myinit
private :: makeCheckImage

integer, private, parameter :: checkImageWidth = 64
integer, private, parameter :: checkImageHeight = 64
integer(kind=GLubyte), private, &
   dimension(0:2,0:checkImageWidth-1,0:checkImageHeight-1) :: checkImage
...
subroutine myinit()
    ...
    call glTexImage2D(GL_TEXTURE_2D, 0, 3, checkImageWidth, &
       checkImageHeight, 0, GL_RGB, GL_UNSIGNED_BYTE, &
        reshape(checkImage,(/3*checkImageWidth*checkImageHeight/)))
...
subroutine display()
    ...
    call glVertex3f(1.0, 1.0, 0.0)
    call glTexCoord2f(1.0, 1.0)
    call glVertex3f(2.41421, 1.0, -1.41421)
    call glTexCoord2f(1.0, 0.0)
    call glVertex3f(2.41421, -1.0, -1.41421)
    call glEnd()
...
integer :: w
    ...
    w = glutCreateWindow("checker")
    call myinit()
    call glutReshapeFunc (myReshape)
    call glutDisplayFunc(display)
    call glutMainLoop()
end program thismain
